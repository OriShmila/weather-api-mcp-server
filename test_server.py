...
async def run_tool_test(
    ...
    try:
        ...
        from weather_api_mcp_server.server import TOOL_FUNCTIONS
        ...
        if test_case.get("should_succeed", True):
            ...
            with open("weather_api_mcp_server/tools.json", "r") as f:
...
async def test_server_components():
    ...
    try:
        ...
        from weather_api_mcp_server.server import load_tool_schemas, TOOL_FUNCTIONS
...
async def run_tool_tests(schemas: Dict[str, Any]):
    ...
    if not os.getenv("WEATHER_API_KEY"):
        print("⚠️  WEATHER_API_KEY not set - skipping API tests")
        for test_case in test_cases:
            results.add_skip(test_case["name"], "No API key")
        return results
